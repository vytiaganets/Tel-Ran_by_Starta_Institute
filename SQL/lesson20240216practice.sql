...
--update обновить
update table_name
set column = new _value, column2 = new_value
where ...

--обновить зарплату сотрудника с именем "Steven" на 2500
set sql_safe_updates = 0;
update employees set salary = 25000 where first_name = 'Steven';
...
--Добавить столбец manager_id для хранения идентификатора
 --менеджера с типом integer и со значением по умолчанию 1
 --default...
 
alter table employees add manager_id int default 1;

--Обновить департамент сотрудника с именем "Adam" на "IT".
update employees set department = 'IT' where first_name = 'Adam';
