...
select product_name as title from phones;
...
select product_name as title, product_count*starting_price as total_sum from phones;
...
select product_name, starting_price, tax, starting_price + starting_price * tax / 100 as final_price from phones;
...
select first_name, last_name, ifnull(phone, 'не определено') 
as phone, ifnull(email, 'не определено') 
as email
from users1;
...
select substring('IPHONE 13 PRO', 2)
